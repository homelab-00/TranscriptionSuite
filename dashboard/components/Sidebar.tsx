...
import type { ConnectionState } from '../src/hooks/useServerStatus';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, onOpenSettings, onOpenAbout, serverStatus = 'inactive', clientStatus = 'inactive' }) => {
                ...
                >
                    ...
                    {(item.hasClientStatus || item.hasServerStatus) && (
                        <div className={`absolute transition-all duration-200 ${collapsed ? 'top-2 right-2' : 'right-3 top-1/2 -translate-y-1/2'}`}>
                            <StatusLight status={item.hasServerStatus ? serverStatus : clientStatus} className={collapsed ? 'w-2 h-2' : ''} animate={!collapsed} />
                        </div>
                    )}
                </button>
            );
