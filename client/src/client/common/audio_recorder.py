...
class AudioRecorder:
    ...
    def _get_supported_sample_rate(self, device_index: int | None) -> int:
        ...
        logger.warning(f"Could not find supported sample rate, using {self.sample_rate} Hz")
        return self.sample_rate
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            # Get a supported sample rate for the device
            supported_rate = self._get_supported_sample_rate(device_index)
            actual_rate = supported_rate
            
            # Store the actual recording rate for resampling later if needed
