...
**Orchestration (internal).** The route delegates to `core/diarization_dispatch.py::transcribe_with_optional_diarization` (GH-274) — the same decision tree every transcription route uses: integrated single-pass when the active backend overrides `STTBackend.transcribe_with_diarization` (WhisperX, VibeVoice-ASR, SenseVoice CAM++), otherwise `parallel_diarize.py::{transcribe_and_diarize, transcribe_then_diarize}` + `speaker_merge.build_speaker_segments` (with the `build_speaker_segments_nowords` segment-level fallback), otherwise plain `engine.transcribe_file`. The one route-specific edge kept in `openai_audio.py` is the failure-tolerance contract above: if the diarization orchestration itself crashes, the route makes one plain-transcription fallback attempt rather than surfacing a 5xx. The client's `prompt` flows through the dispatch's `initial_prompt` parameter.

Internally the dispatch forces `word_timestamps=True` on the two-pass path so `build_speaker_segments` has alignment data to work with. Per-word fields are only *emitted* in response bodies when the client also passed `timestamp_granularities[]=word` — this preserves OpenAI's external contract while satisfying the internal merge prerequisite.
