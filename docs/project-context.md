...
- **NeMo Greek final sigma (ς)**: canary-1b-v2 and parakeet-tdt-0.6b-v3 tokenizers lack U+03C2 entirely (upstream defect, huggingface.co/nvidia/canary-1b-v2/discussions/26). On REAL speech every NeMo model (Canary included) emits NOTHING at the sigma position - the recoverable " ⁇ " unk marker handled by `core/stt/greek_sigma.py` only appears on unnaturally crisp synthetic audio (espeak), verified on real recordings under every decoding config incl. beam len_pen=2. The robust fix is `core/stt/greek_sigma_llm.py`: the configured LLM provider re-inserts sigmas and a deterministic guard accepts ONLY per-word append-ς / final-σ-to-ς edits (config `local_llm.greek_sigma_repair`, applied in `engine.transcribe_file`; live mode and rolling preview deliberately excluded). Do NOT move the hook into `transcribe_audio` - the rolling preview calls it every few seconds. Do NOT try regex σ-to-ς: the model never emits a trailing σ, the letter is simply gone
