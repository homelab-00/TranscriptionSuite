...
- **NeMo Greek final sigma (ς) - WARNING ONLY, by user decision**: canary-1b-v2 and parakeet-tdt-0.6b-v3 tokenizers lack U+03C2 entirely (upstream defect, huggingface.co/nvidia/canary-1b-v2/discussions/26). Greek word endings come out truncated ("σας" -> "σα"); Canary SOMETIMES emits a " ⁇ " unk marker at the sigma position instead (mostly on crisp/synthetic audio - on real speech usually nothing is emitted, verified under every decoding config incl. beam len_pen=2). There is NO app-level repair: automatic fixes (marker rewrite, LLM restoration) were built and deliberately REMOVED - the user chose honest warnings over partial silent correction. Both backends log a once-per-instance warning for Greek output and the dashboard warns via `modelCapabilities.truncatesGreekFinalSigma` (all NeMo families). Do NOT reintroduce repairs without asking; do NOT try regex σ-to-ς (the model never emits a trailing σ)
