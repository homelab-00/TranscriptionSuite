...
### AMD/Intel Vulkan
- **Linux:** Vulkan-capable GPU with `/dev/dri` access; whisper.cpp sidecar (`ghcr.io/loukas-pap/whisper-cpp-vulkan-noavx2` — our no-AVX2 rebuild of upstream `main-vulkan`, so the sidecar runs on pre-Haswell CPUs too)
