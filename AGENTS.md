...
This project is indexed by GitNexus as **TranscriptionSuite** (4068 symbols, 10754 relationships, 285 execution flows).
...
- After UI edits touching CSS classes: `npm run ui:contract:check` from `dashboard/`.
- When modifying files in `.doc-freshness.yaml`: run `node build/scripts/check-doc-freshness.mjs`.
