...
class DiarizationSegment:
    ...
    def merge_with(self, other: "DiarizationSegment") -> "DiarizationSegment":
        ...
        if self.speaker != other.speaker:
            raise ValueError(
                f"Cannot merge segments from different speakers: {self.speaker} != {other.speaker}"
            )
    ...
    def to_rttm(self, file_id: str = "AUDIO") -> str:
        ...
        """
        return f"SPEAKER {file_id} 1 {self.start:.3f} {self.duration:.3f} <NA> <NA> {self.speaker} <NA> <NA>"

    def __str__(self) -> str:
        """String representation of the segment."""
        conf_str = f" (conf: {self.confidence:.2f})" if self.confidence else ""
        return f"[{format_timestamp(self.start)} --> {format_timestamp(self.end)}] {self.speaker}{conf_str}"

    def __repr__(self) -> str:
        """Developer representation of the segment."""
        return f"DiarizationSegment(start={self.start:.3f}, end={self.end:.3f}, speaker='{self.speaker}', confidence={self.confidence})"
